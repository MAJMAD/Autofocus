...
#Special Credits to Michael Oldenburg and Stefan Vorndran for the Autofocus Constrast Algorithm
#Edited 9/20/2022 to include load screens and visual cycles
import cv2
import numpy as np
...
def RunAutofocusDemo(kpos, xstage, zstage, zstage2):
    ...
    while True:
        ...
        # Control Start Procedure 1
        zstage.MOV('1', zstagemin)
        xstage.MOV('1', xstagemin)
        WaitForMotionDone(zstage, '1')
        WaitForMotionDone(xstage, '1')
        
        for step in range(0, xsteps):
            # "Scan" Procedure
            zstage.MOV('1', kpos[step])
            WaitForMotionDone(zstage, '1')
            time.sleep(1/speed)
        ...
        for step in range(0, xsteps):
            # "Scan" Procedure 
            zstage2.MOV('1', kpos[step + xsteps])
            WaitForMotionDone(zstage2, '1')
            time.sleep(1/speed)
...
def Autofocus():
    LoadScreen()
    LoadScreen("Autofocus Demo", 3)
    #############################################
    SetupStartTime = time.time()
    #############################################
    #Connect Stages
    LoadScreen("Connecting Stages")
    try:
        zstage = ConnectTCP(C414host)
        zstage2 = ConnectSerial(E709path,E709baud)
        xstage = ConnectSerial(C413path,C413baud)
    except:
        LoadScreen("Error Detected\nUnable to connect to stages", 3)
        LoadScreen("Exiting Program", 3)
        exit(1)
    LoadScreen("Connection Successful", 3)
    ############################################
    LoadScreen("Initializing Stages", 5)
    try:
        #Servo Control
        ...
        LoadScreen("Error Detected\nUnable to initialize successfully", 3)
        LoadScreen("Exiting Program", 3)
        exit(1)
    LoadScreen("Initialization Successful", 3)
    #############################################
    ...
    ###############################################
    LoadScreen("Preparing Cameras", 3)
    try:
        #Setup Camera
        cap = cv2.VideoCapture(cam1)
        cap2 = cv2.VideoCapture(cam2)
    except:
        LoadScreen("Error Detected\nCamera disconnected", 3)
        LoadScreen("Exiting Program", 3)
        exit(1)
    LoadScreen("Cameras Connected", 3)
    LoadScreen("Moving to start position", 3)
    try:
        #Move Stages to Start Position
        xstage.MOV('1', xstagemin)
        WaitForMotionDone(xstage, '1')
    except:
        LoadScreen("Error Detected\nUnable to move to start position", 3)
        LoadScreen("Exiting Program", 3)
        exit(1)
    ...
    try:
        ...
        #Start Image Capture Z stage
        for steps in range(0, xsteps):
            # Image Capture Procedure
            filename, path = ImageCapture(zstepsize, ImageCaptureStartTime, zstage, cap, 1)
            MoveXStage(xstepsize, xstage)
            filenames.append(filename)
            paths.append(path)
    except:
        LoadScreen("Error Detected\n Unable to complete \nVoice Coil Image Capture", 3)
        LoadScreen("Exiting Program", 3)
        exit(1)
    ...
    try:
        #Start image capture Z stage 2
        for steps in range(0, xsteps):
            # Image Capture Procedure
            filename, path = ImageCapture(zstepsize2, ImageCaptureStartTime, zstage2, cap2, 2)
            MoveXStage(xstepsize, xstage)
            filenames.append(filename)
            paths.append(path)
        ImageCaptureEndTime = time.time()
    except:
        LoadScreen("Error Detected\n Unable to complete \nPiezo Image Capture", 3)
        LoadScreen("Exiting Program", 3)
        exit(1)
    LoadScreen("Done Piezo Image Capture", 3)
    LoadScreen("Processing Data", 3)
    try:
        dp2StartTime = time.time()
        for steps in range(0, xsteps):
            # Data Processing DaC Procedure
            #print("debug1")
            #print("debug2")
            #print(paths)
            ipos = RunAutofocusCoarseStage(paths[steps], zstagemax - zstagemin , ((zstagemax - zstagemin) / 10), StefanContrastRestructuredV25X5A, 2, 2, 1)
            #print('ipos')
            #print(ipos)
            #print("debug3")
            fpos = RunAutofocusFineStage(paths[steps], ((zstagemax - zstagemin) / 10), ((zstagemax - zstagemin) / 100), ipos, StefanContrastRestructuredV25X5A, 2, 2, 1)
            #print("debug4")
            kpos.append(fpos)
        for steps in range(0, xsteps):
            # Data Processing DaC Procedure 
            ipos = RunAutofocusCoarseStage(paths[steps + xsteps], zstage2max - zstage2min , ((zstage2max - zstage2min) / 10), StefanContrastRestructuredV25X5A, 2, 2, 2)
            fpos = RunAutofocusFineStage(paths[steps + xsteps], ((zstage2max - zstage2min) / 10), ((zstage2max - zstage2min) / 100), ipos, StefanContrastRestructuredV25X5A, 2, 2, 2)
            kpos.append(fpos)
        dp2EndTime = time.time()
    except:
        LoadScreen("Error Detected\n Unable to process data", 3)
        LoadScreen("Exiting Program", 3)
        exit(1)
    LoadScreen("Done Image Processing", 3)
    LoadScreen("Outputting live video feed", 3)
